...
//      }
    ...
    val originalInput = input.lines.toList

    // (false: original, true: reversed), (false: original, true: reverse)
    val directions: List[Boolean] =
      List (false, true, false, )

    def rollRocks(input: List[String], direction: Int): List[String] = {
      val columns = input.transpose.map(line => if (directions(direction)) line.reverse else line)
